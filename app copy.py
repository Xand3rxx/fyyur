...
csrf = CSRFProtect()
csrf.init_app(app)
# To avoid the circular import error modesl file must be imported after the db instatiation
from models import *
...
def venues():
    ...
    return json.dumps(Venue.query.all())
...
def create_venue_submission():
    ...
    if form.validate():
        error = False
        try:
            db.session.bulk_save_objects(Venue(json(request.form)))
            db.session.commit()
        except:
            error = True
            db.session.rollback()
        ...
        flash('Venue ' + venue_name + ' failed due to validation error(s)!')
        flash(form.errors)
